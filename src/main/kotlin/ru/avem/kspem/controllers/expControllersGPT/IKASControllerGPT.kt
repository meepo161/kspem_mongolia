...
class IKASControllerGPT : CustomController() {
    ...
    override fun start() {
        ...
        if (isExperimentRunning) {
            val t = model.data.tempOI.value.toDoubleOrDefault(0.0)
            val rtK = tempKoef
            val rtT = 20.0

            try {
                model.data.R1.value =
                    "%.4f".format(Locale.ENGLISH, (model.data.R1.value.toDouble() / (1 + rtK * (t - rtT))))
                model.data.R2.value =
                    "%.4f".format(Locale.ENGLISH, (model.data.R2.value.toDouble() / (1 + rtK * (t - rtT))))
